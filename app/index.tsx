...
export default function Index() {
  ...
  return (
    <View style={container1}>
      {mostrar || placaShow || resultLay ? <Text style={texto}></Text> : <Opciones setMostrar={setMostrar} setShowPlaca={setShowPlaca} />}
      {mostrar && <Coaxial setMostrar={setMostrar} setResultLay={setResultLay} response={response} setResponse={setResponse}  />}
      {placaShow && <Placas setShowPlaca={setShowPlaca} />}
      {
       resultLay && <ResultadosCoaxial response={response} setResultLay={setResultLay} setMostrar={setMostrar}  />
      }
    </View>
  );
}
